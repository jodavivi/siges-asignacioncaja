	...
	try { 
		...
		for (let index = 0; index < aPermisosCaja.length; index++) {
			const element = aPermisosCaja[index]; 
			 ...
			 var dFechaActualHora = new Date(utils.generarDate());
			...
			var sFechaHoraFinal = sFormatFechaHora.replace("FE", sFechaActual).replace("HA", element.HoraFin);
			 ...
			 //Validamos si esta en el rango de fecha permitido
			 if(dFechaActualHora >= element.FechaInicio 
					&& dFechaActualHora <= element.FechaFin){
						//Validamos si esta entre el rango de hora permitido
